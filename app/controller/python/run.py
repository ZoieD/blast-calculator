...
if __name__=='__main__':
	model = make_model()
	model.load_weights(os.getcwd() + '/app/controller/python/modules/pressure_module.h5',by_name=False)
	# arr = [N, lL, hH, LH, Za, LRa]
	x_test_pressure = np.array([[n, ll, hh, lh, za, lra]])
	pred_pressure = model.predict(x_test_pressure)
	print(pred_pressure)

	model.load_weights(os.getcwd() + '/app/controller/python/modules/impulses_module.h5',by_name=False)
	# arr = [N, lL, hH, LH, Za, LRa]
